...
/// </summary>
public class PositionIndicator : Solver
{
    /// <summary>
    ...
    {
        var dist = Vector3.Distance(SolverHandler.TransformTarget.position, DirectionalTarget.position);

        if (DirectionalTarget == null || SolverHandler.TransformTarget == null || dist < Buffer)
        {
            return false;
        }
    ...
    public override void SolverUpdate()
    {
        ...
        GoalPosition = solverReferenceFrame.position + solverReferenceFrame.forward * ViewOffset;

        // Find the rotation from the facing direction to the target object.
        GoalRotation = Quaternion.LookRotation(indicatorDirection) * Quaternion.Euler(90f, 0f, 0f);
