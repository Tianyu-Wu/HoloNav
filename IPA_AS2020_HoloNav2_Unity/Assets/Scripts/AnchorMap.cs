...
public class AnchorMap : MonoBehaviour
{
    ...
    public void InitScene()
    {
        ...
        if (CreatedSpatialAnchors.Count > 0)
        {
            foreach (KeyValuePair<string, SpatialAnchor> anchor in CreatedSpatialAnchors)
            {
                ...
                connectAnchorDropdown.options.Add(new TMP_Dropdown.OptionData() { text = anchor.Value.Name });
    ...
    private async void HandleCreateAnchorSucceeded()
    {
        ...
        connectAnchorDropdown.options.Add(new TMP_Dropdown.OptionData() { text = currentAnchorPosition.SpatialAnchorObject.Name });
        if (currentAnchorPosition.SpatialAnchorObject.Type == anchorTypeRadialSet.ToggleList[0].name)
        {
